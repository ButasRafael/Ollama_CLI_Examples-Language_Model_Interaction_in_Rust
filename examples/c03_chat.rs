...
pub async fn run_chat_req(ollama: &Ollama, chat_req: ChatMessageRequest) -> Result<Option<String>> {
    ...
    let mut current_asst_msg_elements:Vec<String>=Vec::new();
    while let Some(res)=stream.next().await{
        let res=res.map_err(|_|"Error in chat response")?;
        if let Some(msg)=res.message{
            ...
            stdout.flush().await?;
            current_asst_msg_elements.push(msg_content);
        ...
        if let Some(_final_res)=res.final_data{
            stdout.write_all(b"\n").await?;
            stdout.flush().await?;
            let asst_content=current_asst_msg_elements.join(" ");
